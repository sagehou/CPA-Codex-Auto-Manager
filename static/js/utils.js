...
class ToastManager {
    ...
    getIcon(type) {
        return ''; // Removing character icons to comply with "no emoji" request
    }
...
class ThemeManager {
    ...
    updateToggleButtons() {
        ...
        buttons.forEach(btn => {
            btn.innerHTML = this.theme === 'light' ? moonIcon : sunIcon;
...
const accountStatusBadgeMap = {
    active:  { text: '活跃', class: 'status-active' },
    expired: { text: '过期', class: 'status-expired' },
    banned:  { text: '封禁', class: 'status-banned' },
    failed:  { text: '失败', class: 'status-failed' },
};

function getStatusIcon(status) {
    const s = accountStatusBadgeMap[status];
    if (!s) return `<span class="status-badge status-unknown" title="${status || '未知'}">${status || '未知'}</span>`;
    return `<span class="status-badge ${s.class}" title="${s.text}">${s.text}</span>`;
}
            ...
            <div class="modal-content modal-alert">
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-actions">
                        <button class="btn btn-secondary" id="confirm-cancel" style="width: 100px;">取消</button>
                        <button class="btn btn-primary" id="confirm-ok" style="width: 100px;">确认</button>
                    </div>
                </div>
            </div>
        `;
