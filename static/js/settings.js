...
function renderEmailServices(services) {
    ...
    elements.emailServicesTable.innerHTML = services.map(service => `
        <tr data-service-id="${service.id}">
            ...
            <td title="${service.enabled ? '已启用' : '已禁用'}">
                <span class="status-badge ${service.enabled ? 'active' : ''}">${service.enabled ? '启用' : '禁用'}</span>
            </td>
            <td>${service.priority}</td>
            <td>${format.date(service.last_used)}</td>
            <td>
                <div class="action-buttons">
                    <button class="btn btn-ghost btn-sm" onclick="testService(${service.id})" title="测试">测试</button>
                    <button class="btn btn-ghost btn-sm" onclick="toggleService(${service.id}, ${!service.enabled})" title="${service.enabled ? '禁用' : '启用'}">${service.enabled ? '停用' : '激活'}</button>
                    <button class="btn btn-ghost btn-sm" onclick="deleteService(${service.id})" title="删除">删除</button>
                </div>
            </td>
        </tr>
    `).join('');
...
function renderProxies(proxies) {
    ...
    elements.proxiesTable.innerHTML = proxies.map(proxy => `
        <tr data-proxy-id="${proxy.id}">
            ...
            : `<button class="btn btn-ghost btn-sm" onclick="handleSetProxyDefault(${proxy.id})">设默认</button>`
        }
            </td>
            <td title="${proxy.enabled ? '已启用' : '已禁用'}">
                <span class="status-badge ${proxy.enabled ? 'active' : ''}">${proxy.enabled ? '是' : '否'}</span>
            </td>
...
function renderCpaServicesTable(services) {
    ...
    elements.cpaServicesTable.innerHTML = services.map(s => `
        <tr>
            <td>${escapeHtml(s.name)}</td>
            <td style="font-size:0.85rem;color:var(--text-muted);">${escapeHtml(s.api_url)}</td>
            <td style="text-align:center;" title="${s.enabled ? '已启用' : '已禁用'}">
                <span class="status-badge ${s.enabled ? 'active' : ''}">${s.enabled ? '是' : '否'}</span>
            </td>
            <td style="text-align:center;">${s.priority}</td>
            <td style="white-space:nowrap;">
